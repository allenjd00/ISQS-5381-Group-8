...
*Note: This repository currently includes local IPEDS extracts in the project root. If your clone does not include them, download from [IPEDS via NSF](https://nces.ed.gov/ipeds/datacenter/) and place the CSVs in the project root.*

## Pipeline Architecture

The pipeline consists of reproducible Python scripts, orchestrated by a runner:
...
| 07 | `07_build_recommendation_scores.py` | Build recommendation component scores and composite ranking | `analysis_base.csv`, `modeling_ready.csv`, `project_target_unitid.csv` | `recommendation_base_scores.csv`, `recommendation_scoring_report.json` |
| 08 | `08_explore_fulljoin_feature_importance.py` | Full-join feature exploration and importance diagnostics | Raw CSVs + metadata dictionaries | `analysis_fulljoin_explore.csv`, `feature_importance_fulljoin_explore.csv`, `feature_importance_fulljoin_explore_report.json` |
...
```bash
git clone https://github.com/allenjd00/ISQS-5381-Group-8.git
cd ISQS-5381-Group-8
...
If needed, download IPEDS datasets from [IPEDS Data Center](https://nces.ed.gov/ipeds/datacenter/) and place in project root:
...
- `outputs/recommendation_base_scores.csv` – recommendation scores/ranks for app consumption
- `outputs/recommendation_scoring_report.json` – scoring weights and component notes
- `outputs/feature_importance_fulljoin_explore.csv` – extended feature importance export
...
ISQS-5381-Group-8/
├── app/
│   └── streamlit_app.py                 # Streamlit recommendation UI
├── data/
│   ├── panel_costs.csv                  # Tuition panel input for UI
│   └── value_added_by_major.csv         # Value-added by major input
├── docs/
│   └── contrib_inputs/
│       ├── amanda_tuition_ui_mockup.pdf
│       └── pedro_requirements_steps.txt
├── img/                                 # Rendered and reference images
...
│   ├── 07_build_recommendation_scores.py
│   └── 08_explore_fulljoin_feature_importance.py
├── run_app.ps1                          # Optional PowerShell launcher
├── README.md
└── requirements.txt
```
